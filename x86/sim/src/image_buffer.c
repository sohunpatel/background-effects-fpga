...
void image_buffer_init(image_buffer_t *me) {
...
void image_buffer_lock(image_buffer_t *me) { pthread_mutex_lock(&me->mutex); }

void image_buffer_unlock(image_buffer_t *me) {
...
pixel_t *image_buffer_pop(image_buffer_t *me) {
    pixel_t *image =
        malloc(IMAGE_RES_WIDTH * IMAGE_RES_HEIGHT * sizeof(pixel_t));
    return image;
}
